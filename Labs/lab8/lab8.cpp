...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.804, 0.361, 0.361));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(1.000, 0.855, 0.725));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.847, 0.749, 0.847));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform1i(spotlight_loc, (spotlight) ? 1 : 0);

    int cut_off = glGetUniformLocation(shader->program, "cut_off_radians");
    glUniform1f(cut_off, cut_off_radians);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) cut_off_radians += deltaTime;
        if (window->KeyHold(GLFW_KEY_DOWN)) cut_off_radians -= deltaTime;
    }
}
